<x-app-layout>
    ...
    <div class="container mx-auto px-4 mt-8 pb-24">
            ...
            @foreach ($products as $item)
                <form action="{{ route('products.add-to-cart') }}" method="post">
                    ...
                    <div class="product-card bg-white rounded-lg shadow-md hover:shadow-lg transition-all duration-300 ease-in-out">
                        <img src="{{ $item->image_url }}" class="w-full h-48 object-cover rounded-t-lg" alt="Product Image">
                        <div class="p-4">
                            <h5 class="text-xl font-semibold mb-2">{{ $item->name }}</h5>
                            <p class="text-gray-600 mb-4">{{ $item->description }}</p>
                            <div class="flex items-center justify-between">
                                <p class="text-green-500 font-bold">{{ number_format($item->price, 2) }}</p>
                                <button type="submit" class="text-blue-500 hover:text-blue-600">Add to Cart</button>
                            </div>
                        </div>
                    </div>
                </form>
